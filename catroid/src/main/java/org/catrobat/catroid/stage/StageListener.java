...
public class StageListener implements ApplicationListener {
	...
	private Map<Sprite, ShowBubbleActor> bubbleActorMap = new HashMap<>();
        private Map<Sprite, List<Sprite>> clonePool = new HashMap<>();
        private static final int MAX_POOL_SIZE_PER_SPRITE = 50;
	...
	public void cloneSpriteAndAddToStage(Sprite cloneMe) {
		Sprite original = cloneMe.isClone ? cloneMe.myOriginal : cloneMe;
		List<Sprite> pool = clonePool.get(original);
		Sprite copy = null;
		if (pool != null && !pool.isEmpty()) {
			copy = pool.remove(pool.size() - 1);
			copy.setName(original.getName() + "-c" + StageActivity.getAndIncrementNumberOfClonedSprites());
			for (UserVariable var : copy.getUserVariables()) {
				UserVariable origVar = original.getUserVariable(var.getName());
				if (origVar != null) var.setValue(origVar.getValue());
			}
			...
			cloneMe.look.copyTo(copy.look);
		} else {
			copy = new SpriteController().copyForCloneBrick(cloneMe);
			copy.myOriginal = original;
		}
		copy.look.createBrightnessContrastHueShader();
		addCloneActorToStage(stage, stage.getRoot(), cloneMe.look, copy.look);
		if (!sprites.contains(copy)) {
			sprites.add(copy);
		}
	...
	public boolean removeClonedSpriteFromStage(Sprite sprite) {
		...
		if (removedSprite) {
			sprite.look.remove();
			Sprite original = sprite.myOriginal;
			if (original != null) {
				List<Sprite> pool = clonePool.get(original);
				if (pool == null) {
					pool = new ArrayList<>();
					clonePool.put(original, pool);
				}
				if (pool.size() < MAX_POOL_SIZE_PER_SPRITE) {
					pool.add(sprite);
				} else {
					sprite.invalidate();
				}
